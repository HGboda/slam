...
# debugy1 = [0]
# global fig_g
# fig_g = pylab.figure(1)
# ax_g = fig_g.add_subplot(111)
# global manager_g
# manager_g = pylab.get_current_fig_manager()
# # line_g=ax_g.plot(debugx0,debugy0,'*')
...
def writeImage(grid,fileName=None):
    ...
    if(fileName!=None):
        ...
        # im = ImageTk.PhotoImage(Image.open(fileName+'.png'))
        showMap(im)
...
def main(datafile='unlocalized.data',cell_Size=0.1,Typ='1'):
    ...
    # print 'Onew'
    # print Onew
    # print 'Onew[0][0]',Onew[0][0]
    print 'numParticles: %f,Onew[0][1]: %f,Onew[0][2]: %f,Onew[0][3]: %f' %(numParticles,Onew[0][1],Onew[0][2],Onew[0][3])
    ...
    for k in range(len(S)):
        ...
        # print '\n Reading:',k
        Snew = None
        # print 'Type',Type,'len(L[k][1:])',len(L[k][1:])
        if Type=='0' or Type=='-1':
            Snew = L[k][1:]
            # print 'here'
        else:
        ...
        for cnt in range(0,len(Snew),4):
            debugx0.append(float(Snew[cnt]))
            debugy0.append(float(Snew[cnt+1]))
            debugx1.append(float(Snew[cnt+2]))
            debugy1.append(float(Snew[cnt+3]))
        
            # plt.scatter(debugx0,debugy0,marker='*')
        ...
        if(k%interval==0):
            ...
            writeImage(Grid,'C:/Users/ADASlab4/Documents/IPython Notebooks/Maps/Map'+str(10000+(k/interval))) 
    ...
    mp = 'C:/Users/ADASlab4/Documents/IPython Notebooks/Maps/FinalMap-'
    if(Type=='-1'): 
...
def RealtimePloter(arg):
    ...
    global manager_g
    global fig_g
    # print 'RealtimePloter'
    # print debugx1
    # print debugy1
    # line_g[0].set_data(debugx0,debugy0)
    print debugx1[:4]
    line_g[0].set_data(debugx1,debugy1)
    manager_g.canvas.draw()
    manager_g.show()
    # time.sleep(10)
    
def datagenerator(arg):
    global debugx0
    global debugy0
    global debugx1
    global debugy1
    print 'datagenerator',debugx1[:4]


# timer = fig_g.canvas.new_timer(interval=20)
# timer.add_callback(RealtimePloter, ())
# timer.start()
# timer2 = fig_g.canvas.new_timer(interval=20)
# timer2.add_callback(datagenerator, ())
# timer2.start()
# pylab.show()   
...
if(__name__=='__main__'):
    ...
    #     cellSize = sys.argv[2]  
    # Type = sys.argv[3]    
    datafile = 'C:/Users/ADASlab4/Documents/IPython Notebooks/localized.data'
    cellSize = 10.0
    Type = '0'
